...
export default class ExecutableCode {
  /**
   * @param {WebDemoApi} api
   * @param {string|HTMLElement} target
   * @param {{compilerVersion: *}} [config]
   * @param {Object} eventFunctions
   */
  constructor(api, target, config = {}, eventFunctions) {
    const targetNode =
    ...
    const targetPlatform =
      getTargetById(targetNode.getAttribute(ATTRIBUTES.PLATFORM)) ||
  ...
  static create(target, options) {
    ...
    const api = new WebDemoApi({ onRequest: options.onRequest });

    return api.getCompilerVersions().then((versions) => {
      const instances = [];
      ...
      targetNodes.forEach((node) => {
        ...
        if (versions) {
          ...
          instances.push(
            new ExecutableCode(api, node, { compilerVersion }, options),
          );
        } else {
          console.error("Cann't get kotlin version from server");
          instances.push(
            new ExecutableCode(api, node, { highlightOnly: true }),
          );
        }
      });

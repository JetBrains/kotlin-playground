...
const BUG_REPORT_MESSAGE =
  `${ANGLE_BRACKETS_LEFT_HTML}errStream${ANGLE_BRACKETS_RIGHT_HTML}Hey! It seems you just found a bug! \uD83D\uDC1E\n` +
...
export function processJVMOutput(output, theme) {
  ...
  return processedOutput
    .split(BUG_FLAG)
    .join(BUG_REPORT_MESSAGE)
    .split(`${ANGLE_BRACKETS_LEFT_HTML}outStream${ANGLE_BRACKETS_RIGHT_HTML}`)
    .join(`<span class="standard-output ${theme}">`)
    .split(`${ANGLE_BRACKETS_LEFT_HTML}/outStream${ANGLE_BRACKETS_RIGHT_HTML}`)
    .join('</span>')
    .split(`${ANGLE_BRACKETS_LEFT_HTML}errStream${ANGLE_BRACKETS_RIGHT_HTML}`)
    .join(`<span class="error-output ${theme}">`)
    .split(`${ANGLE_BRACKETS_LEFT_HTML}/errStream${ANGLE_BRACKETS_RIGHT_HTML}`)
    .join('</span>');
}
...
export function processJUnitResults(data, onTestPassed, onTestFailed) {
  ...
  for (let testClass in data) {
    ...
    result += listOfResults.reduce((previousTest, currentTest) => {
      ...
      if (
        currentTest.status === TEST_STATUS.ERROR.value ||
        currentTest.status === TEST_STATUS.FAIL.value
      )
        passed = false;
      switch (currentTest.status) {
        case TEST_STATUS.FAIL.value:
          return (
            previousTest +
            buildOutputTestLine(
              TEST_STATUS.FAIL.text,
              currentTest.methodName,
              currentTest.comparisonFailure.message,
            )
          );
        case TEST_STATUS.ERROR.value:
          return (
            previousTest +
            buildOutputTestLine(
              TEST_STATUS.ERROR.text,
              currentTest.methodName,
              currentTest.exception.message,
            )
          );
...
function buildOutputTestLine(status, method, message) {
  ...
  <div class="console-block">
    ...
    <div class="test-${status.toLocaleLowerCase()}">${status}: ${method}${
      message ? ': ' + convertToHtmlTag(message) : ''
    }</div>
  </div>
  `;
}
...
export function processErrors(errors, theme) {
  return errors.reduce((acc, currentValue) => {
    return (
      acc +
      `<div class="console-block">
                        <span class="console-icon attention"></span>
                        <div class="test-fail ${theme}">${escapeHtml(
...
export function showJsException(exception) {
  ...
  if (exception.stack != null) {
    let userStackTrace = exception.stack
      .toString()
      .substr(0, exception.stack.toString().indexOf('at eval (<anonymous>)'));
...
function getSecurityException(exception) {
  if (exception.stackTrace != null) {
    ...
    exception.stackTrace = exception.stackTrace.slice(
      exception.stackTrace.length - 1,
    );
  }
  return exception;
}
